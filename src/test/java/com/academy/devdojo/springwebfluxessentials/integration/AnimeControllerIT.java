...
public class AnimeControllerIT {
	...
	public void listAll_ReturnFluxOfAnime_WhenSuccessful() {
		testClientUser.get().uri("/animes").exchange().expectStatus().is2xxSuccessful().expectBody().jsonPath("$.[0].id")
	...
	public void listAll_Flavor2_ReturnFluxOfAnime_WhenSuccessful() {
		testClientUser.get().uri("/animes").exchange().expectStatus().isOk().expectBodyList(Anime.class).hasSize(1)
				.contains(anime);
	}
	...
	public void findById_ReturnMonoError_WhenEmptyMonoIsReturned() {
		testClientUser.get().uri("/animes{id}", 1).exchange().expectStatus().isNotFound().expectBody().jsonPath("$.status")
	...
	public void save_CreatesAnime_WhenSuccessful() {
		...
		testClientUser.post().uri("/animes").contentType(MediaType.APPLICATION_JSON)
	...
	public void saveBatch_CreatesListOfAnime_WhenSuccessful() {
		...
		testClientUser.post().uri("/animes/batch").contentType(MediaType.APPLICATION_JSON)
	...
	@Test
	@DisplayName("saveBatch returns mono error when one of objects in the list contains null or empty name")
	...
	public void delete_DeleteAnime_WhenSuccessful() {
		testClientUser.delete().uri("/animes/{id}", 1).exchange().expectStatus().isNoContent();
	}
	...
	public void delete_ReturnMonoError_WhenEmptyMonoIsReturned() {
		...
		testClientUser.delete().uri("/animes{id}", 1).exchange().expectStatus().isNotFound().expectBody()
	...
	public void update_SaveUpdatedAnime_WhenSuccessful() {
		testClientUser.put().uri("/animes/{id}", 1).contentType(MediaType.APPLICATION_JSON)
